...
func (s *State) HandleMessage(index int, deleteMessage bool) (result []*State) {
	...
	to := message.To()
	targetNode := s.GetNode(to)
	//get the new nodes(server, client)
	newNodes := targetNode.MessageHandler(message)
	// fmt.Printf("result new nodes%+v\n", newNodes)

	for _, newNode := range newNodes {
		//inherit new state for each new node
		// fmt.Printf("Handle message new node : %+v \n", newNode)
		if deleteMessage {
...
func (s *State) NextStates() []*State {
	...
	for i := range s.Network {
		...
		if s.isDropOff {
			...
			dropOffState.DeleteMessage(i)
			nextStates = append(nextStates, dropOffState)
		}
		...
		// (use HandleMessage)
		if s.isDuplicate {
			// duplicateStates := s.Inherit(HandleDuplicateEvent(message))
			duplicateNewStates := s.HandleMessage(i, false)
			nextStates = append(nextStates, duplicateNewStates...)
		}
	...
	// 	// fmt.Printf("states %+v\n", s)
	// 	fmt.Printf("validate success s1 %+v \n", s.Nodes()["s1"])
	// 	fmt.Printf("validate success s2 %+v \n", s.Nodes()["s2"])
	// 	fmt.Printf("validate success s3 %+v \n", s.Nodes()["s3"])
	// }
	return nextStates
}
...
func (s *State) RandomNextState() *State {
	...
	// fmt.Printf("the roll int %+v \n", len(s.Network)+len(timerAddresses))
	...
	if roll < len(s.Network) {
		...
		x := s.HandleMessage(roll, false)
		// fmt.Printf("this is bug %+v \n, state %+v \n", x, s)
		if len(x) > 0 {
			return x[0]
		}
	}
	...
	node := s.nodes[address]
	timerStates := s.TriggerNodeTimer(address, node)
	return timerStates[0] // Randomly choose one of the timer states
	// fmt.Println(node)
	// return &State{}
}
